...
public class StatPrinter {
	...
	for (int i = 0; i < data.size(); i++) {
	    if (data.get(i).compareTo(m) > 0) {
		m = data.get(i); }//replace m with max number
	}
	return m;
    ...
    {
	//System.out.println("hi"+_frequency);
        return ((i > 0) && ( i < _frequency.size() -1)
		&& (_frequency.get(i).compareTo(_frequency.get(i+1)) > 0)
		&& (_frequency.get(i).compareTo(_frequency.get(i-1)) > 0));
 //this method runs in O(1) time because it does the same amount of work each time
    }
	...
	ArrayList<Integer> modes = new ArrayList<Integer>();
		...
		modes.add(_frequency.get(i)); }//adds if it is a local mode
	}
	return modes;
	//this method runs in O(n) time
    }
	...
	for (int i = 0; i <_frequency.size(); i++) {
	    System.out.println(i +" : "+getStars(_frequency.get(i),longestBar)); 
	}
	//this method runs in O(n) time
    }

    public String getStars(int i, int longestBar) {
	//function to return proportion of stars
        int numStars = longestBar / max(_frequency);
	String retStr = "";
	for (int k = 0; k < i * numStars; k++) {
	    retStr+= "*"; }
	return retStr;
	//this method runs in O(n) time
    }
